...
interface TypeCardBComponentProps {
...
export function TypeCardBComponent({
  editAllegati,
  livello,
  dataConseguimento,
  imgFronte,
        ...
        <div>
          ...
          <div className="grid grid-cols-[auto_1.5fr_auto_1fr] p-4 pl-[10%] items-center gap-x-2">
            ...
            <Select
              value={livelloValue}
              onValueChange={(value) =>
                handleChangeFormAllegati(livelloKey, value)
              }
            >
              <SelectTrigger className="rounded-[3px] bg-[#fff] !text-[14px] text-[#4c4a4a]">
                <SelectValue placeholder="Seleziona" />
              </SelectTrigger>

              <SelectContent>
                {items.map((item) => (
                  <SelectItem key={item.value} value={item.value} className="text-sm py-1">
                    {item.label}
                  </SelectItem>
                ))}
              </SelectContent>
